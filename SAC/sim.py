...
class sim_maze3(sim):
    ...
    def render(self, bullet_lidar):
        img = super().render(bullet_lidar)
        img = lidar_util.imshowTgt("render"+str(self.phisicsClient), 800, 800, np.array([self.tgt_pos-np.array(self.getState()[:2])])/bullet_lidar.maxLen, maxLen=1.0, show=False, preimg=img)
        return img
